...
dados_crua_inicial = dados_crua_inicial.drop(dados_crua_inicial.columns[[0, 2, 4, 5, 6, 7,  10, 11, 12, 13, 14, 15, 16, 18, 19]], axis=1)
...
dados_crua_inicial.columns = ['Nome do Paciente', 'Registro', 'Atendimento', 'Alta', 'Convenio']
dados_crua_inicial = dados_crua_inicial[~dados_crua_inicial['Nome do Paciente'].str.contains("Emitido em:", na=False)]
...
dicionario_convenios = {
    ...
    "PATRONAL-GEAP":'GEAP', "CASU - CAIXA DE ASSISTENCIA A": "CASU", "SASC - SANTA CASA SAUDE COMPLE":"SASC",
...
dados_crua_inicial['Convenio'] = dados_crua_inicial['Convenio'].str.replace('^\d+-', '', regex=True)
dados_crua_inicial
...
for index, linha in dados_crua_inicial.iterrows():
    ...
    if "Medico:" in registro:
        medico_atual = re.sub('^\d+ - ', '', " ".join(registro.split()[1:])).strip()  # Atualiza o médico atual, removendo o código
    else:
...
dados_processados_a_faturar = pd.DataFrame(dados_processados)
...
dados_processados_a_faturar_df.to_excel('C:/Users/ACER/Meu Drive/Hospital Nossa Senhora das Mercês/Dashboard/Projeto Diagnóstico de Faturamento/a_faturar_df.xlsx', index=False)
